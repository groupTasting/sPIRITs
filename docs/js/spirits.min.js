...
function drawChart(ginData, elem, type) {
  ...
  var rateClass = "."+type+"Rating";
  $(rateClass).starRating({
  totalStars: 5,
  initialRating: data.avg,
  readOnly: true,
  starShape: 'rounded',
  starSize: 30,
  ...
  mounted () {
	...
	var tmpGinId = '0'; 
	  ...
	  tmpGinId = id;  
        ...
        window.focus();
        this.blurListener = window.addEventListener('blur', this.blurFrame);  //not really working in firefox
        this.focusInt = setInterval(function(){
          document.activeElement.blur();
          window.focus();   
        }, 500);
        this.blurInt = setInterval(this.blurFrame, 200);
